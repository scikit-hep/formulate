...
def _create_test_type(name, A, B, C, D):
    ...
    class NewTestClass:
        ...
        def test_chain_math(self):
            check_result(
                "{A} + {B} + {C} + {D}", Expression(IDs.ADD, A, B, C, D), **kwargs
            )
            check_result(
                "{A} - {B} - {C} - {D}", Expression(IDs.SUB, A, B, C, D), **kwargs
            )
            check_result(
                "{A} * {B} * {C} * {D}", Expression(IDs.MUL, A, B, C, D), **kwargs
            )
            check_result(
                "{A} / {B} / {C} / {D}", Expression(IDs.DIV, A, B, C, D), **kwargs
            )
            check_result(
                "{A} % {B} % {C} % {D}", Expression(IDs.MOD, A, B, C, D), **kwargs
            )
        ...
        def test_signed_functions(self):
            ...
            check_result("arctan2({A}, {B})", Expression(IDs.ATAN2, A, B), **kwargs)
            check_result(
                "-sqrt({A})", Expression(IDs.MINUS, Expression(IDs.SQRT, A)), **kwargs
            )
            check_result(
                "+sqrt({A})", Expression(IDs.PLUS, Expression(IDs.SQRT, A)), **kwargs
            )
            check_result(
                "- arctan2({A}, {B})",
                Expression(IDs.MINUS, Expression(IDs.ATAN2, A, B)),
                **kwargs
            )
            check_result(
                " + arctan2({A}, {B})",
                Expression(IDs.PLUS, Expression(IDs.ATAN2, A, B)),
                **kwargs
            )
        ...
        def test_math_with_functions(self):
            if isinstance(A, UC):
                ...
                check_result(
                    "sqrt(-{A})",
                    Expression(IDs.SQRT, Expression(IDs.MINUS, A)),
                    **kwargs
                )
                check_result(
                    "sqrt(+ {A})",
                    Expression(IDs.SQRT, Expression(IDs.PLUS, A)),
                    **kwargs
                )
            check_result(
                "sqrt({A} + {B})",
                Expression(IDs.SQRT, Expression(IDs.ADD, A, B)),
                **kwargs
            )
            check_result(
                "arctan2({A} - {B}, {B} % {A})",
                Expression(
                    IDs.ATAN2, Expression(IDs.SUB, A, B), Expression(IDs.MOD, B, A)
                ),
                **kwargs
            )
            check_result(
                "sqrt({A})+sqrt({B})",
                Expression(IDs.ADD, Expression(IDs.SQRT, A), Expression(IDs.SQRT, B)),
                **kwargs
            )
            check_result(
                "sqrt({A})*sqrt({A})",
                Expression(IDs.MUL, Expression(IDs.SQRT, A), Expression(IDs.SQRT, A)),
                **kwargs
            )
            check_result(
                "arctan2({A}, {B})/sqrt({B})",
                Expression(
                    IDs.DIV, Expression(IDs.ATAN2, A, B), Expression(IDs.SQRT, B)
                ),
                **kwargs
            )
        ...
        def test_functions_of_functions(self):
            check_result(
                "sqrt(sqrt({A}))",
                Expression(IDs.SQRT, Expression(IDs.SQRT, A)),
                **kwargs
            )
            check_result(
                "sqrt(arctan2({A}, {B}))",
                Expression(IDs.SQRT, Expression(IDs.ATAN2, A, B)),
                **kwargs
            )
            check_result(
                "arctan2(sqrt({A}), {B})",
                Expression(IDs.ATAN2, Expression(IDs.SQRT, A), B),
                **kwargs
            )
        ...
        def test_nested(self):
            check_result(
                "sqrt(-sqrt({A}))",
                Expression(IDs.SQRT, Expression(IDs.MINUS, Expression(IDs.SQRT, A))),
                **kwargs
            )
            check_result(
                "sqrt(sqrt({A}) + {B})",
                Expression(IDs.SQRT, Expression(IDs.ADD, Expression(IDs.SQRT, A), B)),
                **kwargs
            )
            check_result(
                "sqrt(sqrt({A}) + sqrt({B}))",
                Expression(
                    IDs.SQRT,
                    Expression(
                        IDs.ADD, Expression(IDs.SQRT, A), Expression(IDs.SQRT, B)
                    ),
                ),
                **kwargs
            )
...
TestPosFloats = _create_test_type(
    "TestPosFloats", UC("1.2"), UC("3.4"), UC("4.5"), UC("6.7")
)
TestNegFloats = _create_test_type(
    "TestNegFloats", UC("-1.2"), UC("-3.4"), UC("-4.5"), UC("-6.7")
)
TestMixFloats = _create_test_type(
    "TestMixFloats", UC("1.2"), UC("-3.4"), UC("4.5"), UC("-6.7")
)

TestPosScientific = _create_test_type(
    "TestPosScientific", UC("1e-2"), UC("3.4e5"), UC("6.7e8"), UC("9e10")
)
TestNegScientific = _create_test_type(
    "TestNegScientific", UC("-1e-2"), UC("-3.4e5"), UC("-6.7e8"), UC("-9e10")
)
TestMixScientific = _create_test_type(
    "TestMixScientific", UC("1e-2"), UC("-3.4e5"), UC("6.7e8"), UC("-9e10")
)

TestVariables = _create_test_type(
    "TestVariables",
    Variable("A"),
    Variable("Bee"),
    Variable("C_is_4"),
