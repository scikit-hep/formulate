...
def test_readme():
    ...
    assert (
        momentum.to_root() == "TMath::Sqrt(((X_PX ** 2) + (X_PY ** 2) + (X_PZ ** 2)))"
    )
    ...
    true_numexpr_eval = numexpr_eval(
        "((X_THETA * 0.017453292519943295) > (3.141592653589793 / 4)) & (D_PE > 9.2)"
    )
    assert numexpr_eval(new_selection.to_numexpr()) == pytest.approx(true_numexpr_eval)
