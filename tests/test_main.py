...
def test_get_named_constants():
    ...
    result = parse_args(
        ["--from-numexpr", "(A & B) | sqrt(A) + 5.4**3.141592", "--named-constants"]
    )
    assert result == ""
    result = parse_args(
        ["--from-root", "(A && B) || TMath::Sqrt(A) + 5.4**pi", "--named-constants"]
    )
    assert result == "PI"
...
def test_get_unnamed_constants():
    ...
    result = parse_args(
        ["--from-numexpr", "(A & B) | sqrt(A) + 5.4**3.141592", "--unnamed-constants"]
    )
